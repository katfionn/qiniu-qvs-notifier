...
if __name__ == '__main__':
    ...
    with open('devices.txt', 'r') as file:
        for line in file:
            # 解析参数1、参数2、参数3
            param1, param2, param3 = line.strip().split(':')
            # 执行函数
            ...
            if state == 'online':
                device = param3
                #webhook_url = 'https://oapi.dingtalk.com/robot/send?access_token=c52b63aebdb616e0b1e211025f90e6cca86eb8e1f019cec9863a222ee6bdedf9' # 替换为你的webhook链接，自己老的群的webhook
                webhook_url = 'https://oapi.dingtalk.com/robot/send?access_token=d4443a36075c05dd2014b9015a7809f2b48158b548af8e5e6539a416f103ef51' # 替换为你的webhook链接,内部群的webhook
                #下面四行注释掉，用来过滤出离线的设备，以实现将离线的设备推送给钉钉机器人
                #if send_dingtalk_message(webhook_url, '在线', device):
                    #print(f'成功发送[设备{device} 在线]消息到钉钉机器人。')
                #else:
                    #print(f'发送[设备{device} 在线]消息到钉钉机器人失败。')
            elif state == 'offline':
                device = param3
                webhook_url = 'https://oapi.dingtalk.com/robot/send?access_token=c52b63aebdb616e0b1e211025f90e6cca86eb8e1f019cec9863a222ee6bdedf9' # 替换为你的webhook链接
                #webhook_url = 'https://oapi.dingtalk.com/robot/send?access_token=d4443a36075c05dd2014b9015a7809f2b48158b548af8e5e6539a416f103ef51' # 替换为你的webhook链接
                if send_dingtalk_message(webhook_url, '离线', device):
                    print(f'成功发送[设备{device} 离线]消息到钉钉机器人。')
                else:
                    print(f'发送[设备{device} 离线]消息到钉钉机器人失败。')
            else:
